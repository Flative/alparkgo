...
class Alparkgo(object):
    ...
    def run(self):
        ...
        self.__parse_slack_information(slack_response)
    ...
    def __parse_slack_information(self, slack_response):
    ...
    def __on_message(self):
        def on_message(ws, ws_message):
            ...
            if slack_message.get('type') == 'message':
                self.__receive_message(ws, slack_message)
        return on_message

    def __receive_message(self, ws, slack_message):
        if self.__is_not_alparkgo(slack_message['user']):
            if self.__is_message_from_receiver(slack_message):
                self.__response(ws, slack_message)
            elif self.__is_message_to_alparkgo(slack_message):
                self.__redirect_message_to_receiver(ws, slack_message)

    def __response(self, ws, slack_message):
    ...
    def __redirect_message_to_receiver(self, ws, slack_message):
    ...
    def __is_not_alparkgo(self, user_id):
        return user_id != self.bot_user_id

    def __is_message_from_receiver(self, slack_message):
        return slack_message['channel'] == self.receiver_channel_id

    def __is_message_to_alparkgo(self, slack_message):
